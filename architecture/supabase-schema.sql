...
-- Brand-scoped RLS for all tables
create policy "brands_select_own" on public.brands for select using (clerk_org_id = auth.clerk_org_id());
create policy "brands_service_role" on public.brands for all using (auth.role() = 'service_role');

create policy "territories_select_own" on public.territories for select using (brand_id in (select id from public.brands where clerk_org_id = auth.clerk_org_id()));
create policy "territories_service_role" on public.territories for all using (auth.role() = 'service_role');

create policy "franchisees_select_own" on public.franchisees for select using (brand_id in (select id from public.brands where clerk_org_id = auth.clerk_org_id()));
create policy "franchisees_service_role" on public.franchisees for all using (auth.role() = 'service_role');

create policy "leads_select_own" on public.leads for select using (brand_id in (select id from public.brands where clerk_org_id = auth.clerk_org_id()));
create policy "leads_insert_own" on public.leads for insert with check (brand_id in (select id from public.brands where clerk_org_id = auth.clerk_org_id()));
create policy "leads_service_role" on public.leads for all using (auth.role() = 'service_role');

create policy "initiatives_select_own" on public.initiatives for select using (brand_id in (select id from public.brands where clerk_org_id = auth.clerk_org_id()));
create policy "initiatives_service_role" on public.initiatives for all using (auth.role() = 'service_role');

create policy "events_select_own" on public.agent_events for select using (brand_id in (select id from public.brands where clerk_org_id = auth.clerk_org_id()));
create policy "events_service_role" on public.agent_events for all using (auth.role() = 'service_role');

create policy "memory_select_own" on public.memory_entries for select using (brand_id in (select id from public.brands where clerk_org_id = auth.clerk_org_id()));
create policy "memory_service_role" on public.memory_entries for all using (auth.role() = 'service_role');

create policy "campaigns_select_own" on public.campaigns for select using (brand_id in (select id from public.brands where clerk_org_id = auth.clerk_org_id()));
create policy "campaigns_service_role" on public.campaigns for all using (auth.role() = 'service_role');
